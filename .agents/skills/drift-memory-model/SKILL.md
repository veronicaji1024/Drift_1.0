...
| T1 | Observations | per-branch | 压缩长对话，保留结构化要素（topics/facts/decisions/openQuestions），使跨分支关联和收敛输出成为可能 |
...
Mastra 的 Observer 产出自由文本 observation。Drift 的 Observer 产出结构化 JSON（topics/facts/decisions/openQuestions/currentTask）。

**Why**：自由文本只能被人读或被 LLM 再次处理。结构化输出可以被 Synthesizer 程序化消费（按 topics 做跨分支匹配）、被 Convergence Engine 按字段聚合（所有分支的 decisions 合并为清单）、被 Intent Detector 对比（当前消息 vs 分支 topics）。
...
Stello 的 integration 产出 synthesis（文本）+ insights（推送列表）。Drift 的 T2 是结构化的 GlobalMap，额外包含 branchSummaries（每个分支的主题句 + 与父/根的关系）和 navigationHints（跳转建议 + relevance 分数 + trigger 类型）。

**Why**：GlobalMap 驱动 UI——树状图的节点标签来自 branchSummaries，连线来自 crossBranchInsights，导航建议来自 navigationHints。如果 T2 只是文本，UI 无法程序化消费。
...
- T2 ≈ synthesis + insights，但多了 branchSummaries 和 navigationHints

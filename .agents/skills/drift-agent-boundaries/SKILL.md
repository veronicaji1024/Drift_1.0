...
**做**：读单个分支的 T0 raw messages → 产出结构化 T1 observation（topic/stage/keyPoints/openQuestions/directionSignal）
...
**做**：读所有分支的 T1 observations + 树结构 → 产出 GlobalMap（overallTheme + branchLandscape + crossThemeConnections + explorationCoverage + convergenceReadiness + navigationSuggestions）
...
**做**：读用户行为信号（fork 撤销率、切换频率、输出格式偏好等）→ 产出 UserProfile（thinkingStyle/depthPreference/interactionPattern/focusAreas/responsePreference/confidenceLevel）
...
**做**：判断当前消息是否偏离分支主题 → 输出 continue/fork/backtrack + confidence → 驱动自动 fork

**不做**：
- 不调用 LLM（纯规则 + 关键词 + topic overlap）
- 不自己执行 fork（只返回 IntentResult，由 ForkManager 执行）
- 不做跨分支分析（只看当前分支的 branchContext：topic/stage/keyPoints/directionSignal）
